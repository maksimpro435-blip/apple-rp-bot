...
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, ChatMemberHandler, ConversationHandler, ContextTypes, filters
...
INVITE_LINK = "https://t.me/applerp1"

SEASON_NUMBER = 10
SEASON_NAME = "Сезон 10 (1200)"
SEASON_ARCHIVED_KEY = "season_8_archived"
# =======================================================

logging.basicConfig(level=logging.INFO)

BACKUP_DIR = "backups"
COUNTRIES_FILE = f"countries_s{SEASON_NUMBER}.json"
PENDING_FILE = f"pending_s{SEASON_NUMBER}.json"
RESOURCES_FILE = f"resources_s{SEASON_NUMBER}.json"
REVIEWS_FILE = f"reviews_s{SEASON_NUMBER}.json"
USERS_FILE = f"users_s{SEASON_NUMBER}.json"
ARCHIVE_FILE = "archive.json"
BANNED_FILE = "banned.json"
ADMINS_FILE = "admins.json"
ANKETERS_FILE = "anketers.json"
...
def load_mechanics():
    return load_txt_full("mechanics.txt") or "Механика не найдена."

def load_start_conditions():
    return load_txt_full("start_conditions.txt") or "Стартовые условия не найдены."

def load_government():
    return load_txt_full("government.txt") or "Информация не найдена."

def load_answers():
    answers = {}
    if os.path.exists("bot_answers.txt"):
        with open("bot_answers.txt", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if "|" in line:
                    parts = line.split("|", 1)
                    if len(parts) == 2:
                        answers[parts[0].strip()] = answers.get(parts[0].strip(), []) + [parts[1].strip()]
    return answers
...
anketers = set(load_json(ANKETERS_FILE))
maper_ids = set()
maper_id = None
...
season_archived = archive.get(SEASON_ARCHIVED_KEY, False) if archive else False
country_counter = {SEASON_NUMBER: 0}
if countries:
    country_counter[SEASON_NUMBER] = max(c.get("number", 0) for c in countries.values())
...
def backup_files():
    ...
    for f in [COUNTRIES_FILE, PENDING_FILE, RESOURCES_FILE, ARCHIVE_FILE, REVIEWS_FILE, USERS_FILE, BANNED_FILE, ADMINS_FILE, ANKETERS_FILE, MAPER_FILE, MAPER_ORDERS_FILE, MAPER_DONE_FILE, ADMIN_LOG_FILE, SURVEYS_FILE]:
        if os.path.exists(f):
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "stata_help": await q.edit_message_text("🆘 /start → меню\nВыбери страну 2022 года", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
        ...
        archive[SEASON_ARCHIVED_KEY] = True; save_json(ARCHIVE_FILE, archive)
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    country_counter[SEASON_NUMBER] = country_counter.get(SEASON_NUMBER,0) + 1
    cn = country_counter[SEASON_NUMBER]
        ...
        try:
            ...
            conv_handler = ConversationHandler(
                ...
                states={
                    ...
                    GOV: [CallbackQueryHandler(gov_pick, pattern="^govpick_"), CallbackQueryHandler(gov_info_handler, pattern="^gov_info$")],
                    CONFIRM_GOV: [CallbackQueryHandler(gov_confirm, pattern="^gov_confirm$"), CallbackQueryHandler(gov_back, pattern="^gov_back$"), CallbackQueryHandler(gov_info_handler, pattern="^gov_info$")],
                    RELIGION: [CallbackQueryHandler(rel_pick, pattern="^relpick_")],
                    CONFIRM_RELIGION: [CallbackQueryHandler(rel_confirm, pattern="^rel_confirm$"), CallbackQueryHandler(rel_back, pattern="^rel_back$")],
                    ATHEISM_MODE: [CallbackQueryHandler(atheism_pick, pattern="^atheism_")],
                    CAPITAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_capital)],
                    ETHNICITY: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_ethnicity)],
                    ...
                    CURRENCY_RATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_currency_rate)],
                    EXTRA: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_extra), CallbackQueryHandler(skip_extra, pattern="^skip_extra$")],
                    CONFIRM: [CallbackQueryHandler(change_menu, pattern="^change_something$"), CallbackQueryHandler(submit_to_admin, pattern="^submit_to_admin$")],
                    CHANGE_MENU: [
                        CallbackQueryHandler(change_name_start, pattern="^change_name$"),
                        CallbackQueryHandler(change_spawn_start, pattern="^change_spawn$"),
                        CallbackQueryHandler(change_gov_start, pattern="^change_gov$"),
                        CallbackQueryHandler(change_religion_start, pattern="^change_religion$"),
                        CallbackQueryHandler(change_capital_start, pattern="^change_capital$"),
                        CallbackQueryHandler(change_ethnicity_start, pattern="^change_ethnicity$"),
                        CallbackQueryHandler(change_flag_start, pattern="^change_flag$"),
                        CallbackQueryHandler(change_currency_start, pattern="^change_currency$"),
                    ...
                    CHANGE_GOV: [CallbackQueryHandler(change_gov_done, pattern="^govpick_")],
                    CHANGE_RELIGION: [CallbackQueryHandler(change_religion_done, pattern="^relpick_"), CallbackQueryHandler(change_atheism_done, pattern="^atheism_")],
                    CHANGE_CAPITAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, change_capital_done)],
                    CHANGE_ETHNICITY: [MessageHandler(filters.TEXT & ~filters.COMMAND, change_ethnicity_done)],
                    CHANGE_FLAG: [MessageHandler(filters.PHOTO | filters.Document.ALL, change_flag_done)],
                    CHANGE_CURRENCY: [MessageHandler(filters.TEXT & ~filters.COMMAND, change_currency_done)],
                    EDIT_COUNTRY: [CallbackQueryHandler(edit_name_start, pattern="^edit_name$"), CallbackQueryHandler(edit_flag_start, pattern="^edit_flag$")],
                    EDIT_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_name_done)],
                    EDIT_FLAG: [MessageHandler(filters.PHOTO | filters.Document.ALL, edit_flag_done)],
                    ADMIN_BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, broadcast_message)],
                    ADMIN_BROADCAST_CONFIRM: [CallbackQueryHandler(button_handler, pattern="^admin_broadcast_yes$"), CallbackQueryHandler(button_handler, pattern="^admin_broadcast_no$")],
                    ANKETER_ADD: [MessageHandler(filters.TEXT & ~filters.COMMAND, anketer_add_id)],
                },
                fallbacks=[CallbackQueryHandler(cancel_create, pattern="^back_to_menu$"), CommandHandler("cancel", cancel_broadcast)],
            )

            review_handler = ConversationHandler(
                entry_points=[CallbackQueryHandler(write_review_start, pattern="^write_review$")],
                states={REVIEW_WRITE: [MessageHandler(filters.TEXT & ~filters.COMMAND, review_received)], REVIEW_RATING: [CallbackQueryHandler(review_rating, pattern="^rate_")]},
                fallbacks=[CallbackQueryHandler(cancel_review, pattern="^reviews_menu$")],
            )

            app.add_handler(CommandHandler("start", start))
            app.add_handler(CommandHandler("admin", admin_command))
            app.add_handler(CommandHandler("adm", mod_command))
            app.add_handler(CommandHandler("map", maper_command))
            app.add_handler(CommandHandler("addadmin", addadmin_command))
            app.add_handler(CommandHandler("tempban", tempban_command))
            app.add_handler(CommandHandler("stata", stata_command))
            app.add_handler(CommandHandler("info", info_command))
            app.add_handler(CommandHandler("menu", menu_command))
            app.add_handler(ChatMemberHandler(welcome_new_member, chat_member_types=ChatMemberHandler.CHAT_MEMBER))
            app.add_handler(ChatMemberHandler(member_banned, chat_member_types=ChatMemberHandler.CHAT_MEMBER))
            app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.GROUPS, chat_message))
            ...
            try:
                app.job_queue.run_repeating(periodic_promo_personal, interval=14400, first=10)
                app.job_queue.run_repeating(periodic_promo_chat, interval=14400, first=30)
            except: pass
