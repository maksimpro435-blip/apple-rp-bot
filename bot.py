        ...
        try:
            ...
            conv_handler = ConversationHandler(
                ...
                states={
                    ...
                    h.ADMIN_BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, a.broadcast_message)],
                ...
                fallbacks=[CallbackQueryHandler(h.cancel_create, pattern="^back_to_menu$"), CommandHandler("cancel", a.cancel_broadcast)],
            )
            ...
            app.add_handler(CallbackQueryHandler(a.admin_approve, pattern="^approve_"))
            app.add_handler(CallbackQueryHandler(a.admin_reject_menu, pattern="^reject_menu_"))
            app.add_handler(CallbackQueryHandler(a.admin_reject_confirm, pattern="^reject_"))
            app.add_handler(CallbackQueryHandler(a.admin_cancel_reject, pattern="^cancel_reject_"))
            app.add_handler(CallbackQueryHandler(h.button_handler))

            try:
                app.job_queue.run_repeating(a.periodic_promo_personal, interval=14400, first=10)
                app.job_queue.run_repeating(a.periodic_promo_chat, interval=14400, first=30)
            except: pass
