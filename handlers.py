...
NAME, SPAWN, GOV, CONFIRM_GOV, FLAG, EXTRA, CONFIRM, CHANGE_MENU, CHANGE_NAME, CHANGE_SPAWN, CHANGE_GOV, CHANGE_FLAG, CHANGE_EXTRA = range(13)
...
def gov_keyboard():
    kb = []
    for g in GOV_TYPES:
        kb.append([InlineKeyboardButton(g, callback_data=f"govpick_{g}")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🍎 <b>Яблочное РП! — {season_name}</b>\n\n🌍 1698 год. Реальные страны.\n⚔️ Выбери свою страну и приведи её к величию!\n<i>Готов?</i>"
    kb = [
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        try:
            ...
            if found:
                text = f"🏛 №{found['number']}\n📛 {found['name']}\n👤 @{found['username']}\n📅 {found['date']}\n🏛 Госстрой: {found.get('gov','Не указан')}\n📝 {found.get('extra_info','Нет')}"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "about_game": await q.edit_message_text("🍎 Лучшее РП!\n👑 ИМПЕРАТОР создал мир.\n🌍 1698 год — эпоха реальных стран.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
        ...
        sid = str(uid)
        if sid in countries:
            c = countries[sid]; text = f"🏛 {c['name']}\n🔢 №{c['number']}\n📅 {c['date']}\n🏛 Госстрой: {c.get('gov','Не указан')}\n🪨 " + ", ".join(resources_data.get(sid, []))
    ...
    elif d == "stata_seasons": await q.edit_message_text("📅 Сезон 1 (1200)\nСезон 2 (1991)\nСезон 3 (2008)\nСезон 4 (1991)\n🌍 Сезон 5 (1698) — текущий", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
    elif d == "stata_help": await q.edit_message_text("🆘 /start → меню\nНапиши реальную страну 1698 года", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
        ...
        if uid != ADMIN_ID: return
        await q.edit_message_text("🌍 " + "\n".join(f"🏛 {c['name']} | @{c['username']} | ID:{u} | 🏛{c.get('gov','?')}" for u, c in countries.items()) if countries else "Нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="admin_menu")]]), parse_mode="HTML")
        ...
        archive["season_5_archived"] = True; save_json(ARCHIVE_FILE, archive)
        ...
        if u in countries:
            c = countries[u]
            await q.edit_message_text(f"🏛 {c['name']}\n👤 @{c['username']}\n🔢 №{c['number']}\n📅 {c['date']}\n🏛 Госстрой: {c.get('gov','Не указан')}\n📝 {c.get('extra_info','Нет')}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="maper_countries")]]), parse_mode="HTML")
        ...
        if u in pending_countries:
            ...
            await q.edit_message_text(f"📋 {data['name']}\n👤 @{data['username']}\n🏛 Госстрой: {data.get('gov','Не указан')}\n📝 {data.get('extra_info','Нет')}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅", callback_data=f"approve_{u}"),InlineKeyboardButton("❌", callback_data=f"reject_menu_{u}")]]), parse_mode="HTML")
...
async def start_create_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text("🏛 <b>Шаг 1/5:</b> Напиши название своей страны:", parse_mode="HTML"); return NAME

async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["country_name"] = update.message.text
    await update.message.reply_html("🗺 <b>Шаг 2/5:</b> Укажи страну (название реальной страны 1698 года или описание/фото территории).\n\n<i>Можно написать название страны или загрузить фото/карту.</i>")
    return SPAWN
...
async def get_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        return SPAWN
    await update.message.reply_html("🏛 <b>Шаг 3/5:</b> Выбери государственный строй:", reply_markup=gov_keyboard())
    return GOV
...
async def gov_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text("🏛 Шаг 3/5: Выбери государственный строй:", reply_markup=gov_keyboard()); return GOV
...
async def gov_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_html("🏴 <b>Шаг 4/5:</b> Отправь флаг страны <b>(ОБЯЗАТЕЛЬНО)</b>:"); return FLAG
...
async def get_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("📝 <b>Шаг 5/5:</b> Напиши дополнение или нажми «Пропустить»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra")]])); return EXTRA
...
async def show_summary(update, context):
    ...
    s = "✅" if context.user_data.get("spawn_photo") or context.user_data.get("spawn_text") else "❌"
    g = context.user_data.get("gov","Не выбран")
    f = "✅" if context.user_data.get("flag_photo") else "❌"
    e = context.user_data.get("extra_info","") or "Нет"
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    country_counter[5] = country_counter.get(5,0) + 1; cn = country_counter[5]; now = datetime.now().strftime("%d.%m.%Y %H:%M")
    countries[uid] = {"name": data["name"], "number": cn, "username": data["username"], "date": now, "gov": data.get("gov",""), "extra_info": data.get("extra_info","")}; save_json(COUNTRIES_FILE, countries)
    ...
    await q.edit_message_text(f"✅ <b>{data['name']}</b> одобрена! (№{cn})\n🏛 Госстрой: {data.get('gov','?')}\n🪨 {', '.join(res)}", parse_mode="HTML")
    try: await context.bot.send_message(chat_id=int(uid), text=f"🎉 <b>{data['name']}</b> создана!\n🔢 №{cn}\n🏛 Госстрой: {data.get('gov','?')}\n🪨\n" + "\n".join(f"• {r}" for r in res), parse_mode="HTML")
    except: pass
    if maper_ids:
        maper_orders[uid] = {"name": data["name"], "username": data["username"], "spawn_text": data.get("spawn_text",""), "spawn_photo": data.get("spawn_photo"), "flag_photo": data.get("flag_photo"), "gov": data.get("gov",""), "extra_info": data.get("extra_info",""), "date": now}
