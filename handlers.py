...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "gov_info": await q.edit_message_text(load_government(), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="gov_back")]]), parse_mode="HTML")
    elif d == "reviews_menu":
    ...
    elif d == "back_to_season": await q.edit_message_text(f"📅 {season_name}", reply_markup=season_menu(), parse_mode="HTML")
    elif d == "my_resources":
        sid = str(uid)
        ...
        await q.edit_message_text(f"✅ {success}/{total}"); context.user_data.clear(); return ConversationHandler.END
        ...
        archive["seasons"][str(season_number)] = {"season":season_number,"name":season_name,"countries":countries.copy(),"users":len(all_users),"archived_at":datetime.now().strftime("%d.%m.%Y %H:%M")}
        ...
        if uid != ADMIN_ID: return
        text = "📋 <b>Анкетчики:</b>\n\n" + "\n".join(f"• ID:{a}" for a in anketers) if anketers else "Нет."
        ...
        if oid in maper_orders:
            data = maper_orders.pop(oid); maper_done[oid] = data
...
async def anketer_add_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ {new_id}")
...
async def review_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    r = int(q.data.split("_")[1]); u = q.from_user
    reviews[str(u.id)] = {"username": u.username or f"user_{u.id}", "review": context.user_data.get("review_text",""), "rating": r, "date": datetime.now().strftime("%d.%m.%Y %H:%M")}
    save_json(REVIEWS_FILE, reviews)
    await q.edit_message_text(f"🌟 {'⭐'*r}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="back_to_menu")]]), parse_mode="HTML")
...
async def gov_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"✅ Госстрой: <b>{context.user_data['gov']}</b>", parse_mode="HTML")
    await q.message.reply_html("🕌 <b>Шаг 4/8:</b> Выбери религию:\n\n<i>Это основная религия страны, не значит что все 100% её исповедуют.</i>", reply_markup=religion_keyboard())
    return RELIGION
...
async def rel_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"✅ Религия: <b>{rel}</b>", parse_mode="HTML")
...
async def get_capital(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("👥 <b>Шаг 6/8:</b> Напиши какой народ населяет страну:")
    return ETHNICITY

async def eth_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["ethnicity"] = update.message.text
    await update.message.reply_html("🏴 <b>Шаг 7/8:</b> Отправь флаг (ОБЯЗАТЕЛЬНО):")
    return FLAG
...
async def get_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("💱 <b>Шаг 8/8:</b> Напиши название валюты.\n\n<i>Нельзя доллар или реальные валюты. Если реальная страна — напиши «авто».</i>")
    return CURRENCY
...
async def get_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("💱 Курс к доллару (минимум 10):")
...
async def submit_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ADMIN_ID:
        try:
            ...
            admin_kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅", callback_data=f"approve_{u.id}"),InlineKeyboardButton("❌", callback_data=f"reject_menu_{u.id}")]])
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in pending_countries: await q.edit_message_text("❌"); return
    data = pending_countries.pop(uid); save_json(PENDING_FILE, pending_countries)
    country_counter[7] = country_counter.get(7,0) + 1; cn = country_counter[7]
    countries[uid] = {**data, "number": cn, "date": datetime.now().strftime("%d.%m.%Y %H:%M")}
...
async def admin_reject_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("❌ Причина:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📏 Спавн", callback_data=f"reject_{uid}_1")],[InlineKeyboardButton("📛 Название", callback_data=f"reject_{uid}_2")],[InlineKeyboardButton("💩 Бред", callback_data=f"reject_{uid}_3")]]), parse_mode="HTML")
...
async def admin_reject_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in pending_countries:
        pending_countries.pop(uid); save_json(PENDING_FILE, pending_countries)
    rt = reasons.get(rc,"?")
...
async def cancel_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ Отменено."); context.user_data.clear(); return ConversationHandler.END
