        ...
        try:
            ...
            if found:
                text = (
                    ...
                    f"💱 Валюта: {found.get('currency','Не указана')} ({found.get('currency_rate','—')})\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "stata_help": await q.edit_message_text("🆘 /start → меню\nВыбери страну 1936 года", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
        ...
        if u in pending_countries:
            ...
            if data.get("spawn_photo"): await context.bot.send_photo(chat_id=uid, photo=data["spawn_photo"])
            if data.get("flag_photo"): await context.bot.send_photo(chat_id=uid, photo=data["flag_photo"])
...
async def gov_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🏛 Ты выбрал: <b>{context.user_data['gov']}</b>\n\nПодтверди или узнай больше."
...
async def gov_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
...
async def gov_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_html("🕌 <b>Шаг 4/8:</b> Выбери государственную религию.\n\n<i>Это основная религия, не значит что 100% населения её исповедуют.</i>", reply_markup=religion_keyboard())
    return RELIGION
...
async def rel_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rel == "atheism":
        ...
        await q.edit_message_text("🚫 <b>Без религии</b>\n\n<i>Внимание: соц. связь населения ниже.</i>\n\nВыбери режим:", reply_markup=atheism_keyboard(), parse_mode="HTML")
        return ATHEISM_MODE
...
async def atheism_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("✅ Выбрано", parse_mode="HTML")
    await q.message.reply_html("🏙 <b>Шаг 5/8:</b> Напиши столицу:")
    return CAPITAL
...
async def get_ethnicity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("🏴 <b>Шаг 7/8:</b> Отправь флаг страны <b>(ОБЯЗАТЕЛЬНО)</b>:")
    return FLAG
...
async def get_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("💱 <b>Шаг 8/8:</b> Напиши название валюты.\n\n<i>• Нельзя доллар или реальные валюты.\n• Напиши «авто» для автовалюты.\n• Можно придумать свою.</i>")
    return CURRENCY
...
async def submit_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ADMIN_ID:
        try:
            ...
            if data.get("spawn_photo"): await context.bot.send_photo(chat_id=ADMIN_ID, photo=data["spawn_photo"])
...
async def admin_reject_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("❌ Причина:", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("📏 Спавн", callback_data=f"reject_{uid}_1")],
        [InlineKeyboardButton("📛 Название", callback_data=f"reject_{uid}_2")],
        [InlineKeyboardButton("💩 Бред", callback_data=f"reject_{uid}_3")],
