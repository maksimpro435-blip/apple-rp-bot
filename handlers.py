...
ETHNICITIES = [
    "Русские", "Англичане", "Французы", "Немцы", "Испанцы", "Итальянцы",
    "Арабы", "Турки", "Китайцы", "Японцы", "Корейцы", "Индийцы",
    "Персы", "Греки", "Поляки", "Украинцы", "Белорусы", "Сербы",
    "Хорваты", "Чехи", "Словаки", "Венгры", "Румыны", "Болгары",
    "Шведы", "Норвежцы", "Датчане", "Финны", "Голландцы", "Португальцы",
    "Бразильцы", "Мексиканцы", "Аргентинцы", "Американцы", "Канадцы",
    "Австралийцы", "Египтяне", "Нигерийцы", "Эфиопы", "Казахи",
    "Узбеки", "Таджики", "Киргизы", "Туркмены", "Азербайджанцы",
    "Армяне", "Грузины", "Монголы", "Вьетнамцы", "Тайцы"
]
...
def ethnicity_keyboard():
    return InlineKeyboardMarkup([[InlineKeyboardButton(e, callback_data=f"ethpick_{e}")] for e in ETHNICITIES])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🍎 <b>Яблочное РП</b>\n\n"
        "Если ты в РП впервые — переходи по ссылке и заходи в РП:\n"
        "🔗 https://t.me/applerp1\n\n"
        "Если ты уже в РП — напиши /menu чтобы продолжить."
    )
    kb = [
...
async def send_survey(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(chat_id=context.job.chat_id, text="📋 <b>Откуда вы узнали о Яблочном РП?</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
    except: pass
...
async def survey_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("✅ Спасибо за ответ!")
...
async def stata_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("🍎 <b>Я бот Яблочного РП!</b>", reply_markup=stata_keyboard())
    ...
    if args:
        try:
            ...
            if found:
                text = (
                    f"🏛 №{found['number']}\n📛 {found['name']}\n👤 @{found['username']}\n📅 {found['date']}\n"
                    f"🏛 Госстрой: {found.get('gov','Не указан')}\n"
                    f"🕌 Религия: {found.get('religion','Не указана')}\n"
                    f"🏙 Столица: {found.get('capital','Не указана')}\n"
                    f"👥 Народ: {found.get('ethnicity','Не указан')}\n"
                    f"💱 Валюта: {found.get('currency','Не указана')} (курс: {found.get('currency_rate','—')})\n"
                    f"📝 {found.get('extra_info','Нет')}"
                )
        ...
        text = "🌍\n" + ("\n".join(f"🔢 №{c['number']} — 🏛 {c['name']}" for c in countries.values()) if countries else "Нет стран.") + "\n💡 /info 1"
        await update.message.reply_html(text)
...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html(f"🍎 <b>{season_name}</b>", reply_markup=InlineKeyboardMarkup(kb))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "about_game": await q.edit_message_text("🍎 Лучшее РП!\n👑 ИМПЕРАТОР создал мир.\n📅 2008 год.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "gov_info": await q.edit_message_text(load_government(), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад к выбору", callback_data="gov_back")]]), parse_mode="HTML")
    elif d == "reviews_menu":
        ...
        if sid in countries:
            ...
            text = (
                f"🏛 {c['name']}\n🔢 №{c['number']}\n📅 {c['date']}\n"
                f"🏛 Госстрой: {c.get('gov','Не указан')}\n"
                f"🕌 Религия: {c.get('religion','Не указана')}\n"
                f"🏙 Столица: {c.get('capital','Не указана')}\n"
                f"👥 Народ: {c.get('ethnicity','Не указан')}\n"
                f"💱 Валюта: {c.get('currency','Не указана')}\n"
                f"🪨 " + ", ".join(resources_data.get(sid, []))
            )
    ...
    elif d == "stata_seasons": await q.edit_message_text("📅 Сезон 1 (1200)\nСезон 2 (1991)\nСезон 3 (2008)\nСезон 4 (1991)\nСезон 5 (1698)\n🌍 Сезон 6 (2008) — текущий", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
    elif d == "stata_help": await q.edit_message_text("🆘 /start → меню\nВыбери реальную страну 2008 года", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
        ...
        if uid != ADMIN_ID: return
        text = "🌍\n" + "\n".join(f"🏛 {c['name']} | @{c['username']} | ID:{u} | 🏛{c.get('gov','?')} | 🕌{c.get('religion','?')}" for u, c in countries.items()) if countries else "Нет."
        await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="admin_menu")]]), parse_mode="HTML")
        ...
        if uid != ADMIN_ID: return
        await q.edit_message_text("📦", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 Архивировать", callback_data="admin_archive_season")],[InlineKeyboardButton("📂 Смотреть", callback_data="admin_view_archive")],[InlineKeyboardButton("🔙", callback_data="admin_menu")]]), parse_mode="HTML")
        ...
        archive["season_6_archived"] = True; save_json(ARCHIVE_FILE, archive)
        ...
        if uid != ADMIN_ID: return
        text = "📋 <b>Анкетчики:</b>\n\n" + "\n".join(f"• @{all_users.get(str(a),{}).get('username',a)} | ID:{a}" for a in anketers) if anketers else "Нет анкетчиков."
        kb = [[InlineKeyboardButton("➕ Добавить анкетчика", callback_data="anketer_add")]]
        kb += [[InlineKeyboardButton(f"❌ Убрать {a}", callback_data=f"anketer_remove_{a}")] for a in anketers]
        kb.append([InlineKeyboardButton("🔙", callback_data="admin_menu")])
        await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
    elif d == "anketer_add":
        if uid != ADMIN_ID: return
        await q.edit_message_text("📋 Отправь ID анкетчика:", parse_mode="HTML"); return ANKETER_ADD
    elif d.startswith("anketer_remove_"):
        if uid != ADMIN_ID: return
        a = int(d.replace("anketer_remove_",""))
        await q.edit_message_text(f"⚠️ Убрать анкетчика {a}?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Да", callback_data=f"anketer_confirm_remove_{a}")],[InlineKeyboardButton("❌ Нет", callback_data="admin_anketers")]]), parse_mode="HTML")
        ...
        await q.edit_message_text(f"✅ Анкетчик {a} убран!")
        try: await context.bot.send_message(chat_id=a, text="❌ Вы больше не получаете анкеты.")
        except: pass
        ...
        if uid not in maper_ids: return
        text = "📋 " + "\n".join(f"🏛 {d['name']} | @{d['username']} | 🏛{d.get('gov','?')}" for d in maper_orders.values()) if maper_orders else "Нет."
        ...
        if not countries: await q.edit_message_text("🌍 Нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="maper_menu")]]), parse_mode="HTML")
            ...
            kb = [[InlineKeyboardButton(f"🏛 {c['name']} | @{c['username']}", callback_data=f"maper_view_{u}")] for u, c in countries.items()] + [[InlineKeyboardButton("🔙", callback_data="maper_menu")]]
        ...
        if u in countries:
            ...
            if u in resources_data: await context.bot.send_message(chat_id=uid, text="🪨\n" + "\n".join(f"• {r}" for r in resources_data[u]))
        ...
        if uid not in admins_list: return
        text = "📋 " + "\n".join(f"🏛 {d['name']} | @{d['username']} | 🏛{d.get('gov','?')}" for d in pending_countries.values()) if pending_countries else "Нет."
        ...
        if u in pending_countries:
            ...
            text = (
                f"📋 {data['name']}\n👤 @{data['username']}\n"
                f"🏛 Госстрой: {data.get('gov','Не указан')}\n"
                f"🕌 Религия: {data.get('religion','Не указана')}\n"
                f"🏙 Столица: {data.get('capital','Не указана')}\n"
                f"👥 Народ: {data.get('ethnicity','Не указан')}\n"
                f"💱 Валюта: {data.get('currency','Не указана')}\n"
                f"📝 {data.get('extra_info','Нет')}"
            )
            ...
            elif data.get("spawn_text"): await context.bot.send_message(chat_id=uid, text=f"🗺 {data['spawn_text']}")
            if data.get("flag_photo"): await context.bot.send_photo(chat_id=uid, photo=data["flag_photo"], caption="🏴")
    elif d == "mod_countries":
        if uid not in admins_list: return
        await q.edit_message_text("🌍 " + "\n".join(f"🏛 {c['name']} | @{c['username']} | №{c['number']} | 🏛{c.get('gov','?')}" for c in countries.values()) if countries else "Нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="mod_menu")]]), parse_mode="HTML")
    elif d == "mod_stats":
        if uid not in admins_list: return
...
async def anketer_add_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Анкетчик {new_id} добавлен!")
        try: await context.bot.send_message(chat_id=new_id, text="📋 Вы назначены анкетчиком! Вам будут приходить заявки на страны.")
        except: pass
...
async def edit_name_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sid in countries:
        old = countries[sid]["name"]; countries[sid]["name"] = update.message.text; save_json(COUNTRIES_FILE, countries)
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"✏️ «{old}» → «{update.message.text}»")
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if nm.status == "member" and not nm.user.is_bot:
        ...
        msg = random.choice(welcomes) if welcomes else "👑 Новый правитель!"
        try: await context.bot.send_message(chat_id=update.chat_member.chat.id, text=msg, parse_mode="HTML")
        except: pass
...
async def start_create_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text("🏛 <b>Шаг 1/9:</b> Напиши название своей страны:", parse_mode="HTML"); return NAME

async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["country_name"] = update.message.text
    await update.message.reply_html("🗺 <b>Шаг 2/9:</b> Укажи страну (реальная страна 2008 года или описание/фото территории).\n\n<i>Можно написать название страны или загрузить фото/карту.</i>")
    return SPAWN
...
async def get_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_html("❌ Отправь название страны или фото!")
        return SPAWN
    await update.message.reply_html("🏛 <b>Шаг 3/9:</b> Выбери государственный строй:", reply_markup=gov_keyboard())
    return GOV

async def gov_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    gov = q.data.replace("govpick_","")
    context.user_data["gov"] = gov
    text = f"🏛 Ты выбрал: <b>{gov}</b>\n\nПодтверди выбор или узнай больше."
    ...
    await q.edit_message_text(text, reply_markup=kb, parse_mode="HTML"); return CONFIRM_GOV

async def gov_info_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text(load_government(), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад к выбору", callback_data="gov_back")]]), parse_mode="HTML")
    return CONFIRM_GOV

async def gov_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text("🏛 Шаг 3/9: Выбери государственный строй:", reply_markup=gov_keyboard()); return GOV

async def gov_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text(f"✅ Выбрано: <b>{context.user_data.get('gov','?')}</b>", parse_mode="HTML")
    await q.message.reply_html(
        "🕌 <b>Шаг 4/9:</b> Выбери государственную религию.\n\n"
        "<i>Это не значит что вся страна на 100% исповедует её. Это основная религия государства.</i>",
...
async def rel_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rel == "atheism":
        ...
        await q.edit_message_text(
            "🚫 <b>Государство без религии</b>\n\n"
            "<i>Внимание: в странах без религии социальная связь населения обычно ниже. "
            "Это может повлиять на стабильность государства.</i>\n\n"
            "Выбери режим:",
            reply_markup=atheism_keyboard(),
            parse_mode="HTML"
        )
        return ATHEISM_MODE
    context.user_data["religion"] = rel
    text = f"🕌 Ты выбрал: <b>{rel}</b>\n\nПодтверди выбор."
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Подтвердить", callback_data="rel_confirm")],
        [InlineKeyboardButton("🔄 Выбрать другую", callback_data="rel_back")]
    ])
    await q.edit_message_text(text, reply_markup=kb, parse_mode="HTML"); return CONFIRM_RELIGION

async def atheism_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    idx = int(q.data.replace("atheism_",""))
    context.user_data["atheism_mode"] = ATHEISM_MODES[idx]
    context.user_data["religion"] = f"Атеизм ({ATHEISM_MODES[idx]})"
    await q.edit_message_text(f"✅ Выбрано: <b>{ATHEISM_MODES[idx]}</b>", parse_mode="HTML")
    await q.message.reply_html("🏙 <b>Шаг 5/9:</b> Напиши название столицы вашей страны:")
    return CAPITAL

async def rel_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text(f"✅ Выбрано: <b>{context.user_data.get('religion','?')}</b>", parse_mode="HTML")
    await q.message.reply_html("🏙 <b>Шаг 5/9:</b> Напиши название столицы вашей страны:")
    return CAPITAL

async def rel_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text("🕌 Шаг 4/9: Выбери религию:", reply_markup=religion_keyboard()); return RELIGION

async def get_capital(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["capital"] = update.message.text
    await update.message.reply_html("👥 <b>Шаг 6/9:</b> Выбери основной народ, населяющий страну:", reply_markup=ethnicity_keyboard())
    return ETHNICITY

async def eth_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    context.user_data["ethnicity"] = q.data.replace("ethpick_","")
    await q.edit_message_text(f"👥 Выбрано: <b>{context.user_data['ethnicity']}</b>", parse_mode="HTML")
    await q.message.reply_html("🏴 <b>Шаг 7/9:</b> Отправь флаг страны <b>(ОБЯЗАТЕЛЬНО)</b>:")
    return FLAG
...
async def get_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    else: await update.message.reply_html("❌ Флаг обязателен! Отправь фото или документ."); return FLAG
    await update.message.reply_html(
        "💱 <b>Шаг 8/9:</b> Напиши название своей валюты.\n\n"
        "<i>• Нельзя использовать доллар или реальные валюты существующих стран.\n"
        "• Если сезон про реальные страны — нажми «Пропустить», валюта будет автоматически.\n"
        "• Можно придумать свою валюту.</i>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_currency")]])
    )
    return CURRENCY

async def get_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["currency"] = update.message.text
    await update.message.reply_html("💱 <b>Курс валюты:</b>\n\nНапиши сколько твоя валюта стоит в долларах.\n<i>Минимум 10 (например: 10, 15, 20, 39, 100). Нельзя меньше 10.</i>")
    return CURRENCY_RATE

async def skip_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    context.user_data["currency"] = "Автоматически (по стране)"
    context.user_data["currency_rate"] = "—"
    await q.edit_message_text("💱 Валюта: автоматически", parse_mode="HTML")
    await q.message.reply_html("📝 <b>Шаг 9/9:</b> Напиши дополнение или нажми «Пропустить»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra")]]))
    return EXTRA
...
async def get_currency_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if rate < 10:
            await update.message.reply_html("❌ Минимум 10! Напиши число от 10 и выше.")
            return CURRENCY_RATE
        context.user_data["currency_rate"] = str(rate)
    except:
        await update.message.reply_html("❌ Напиши число! Например: 15")
        return CURRENCY_RATE
    await update.message.reply_html("📝 <b>Шаг 9/9:</b> Напиши дополнение или нажми «Пропустить»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra")]]))
    return EXTRA
...
async def show_summary(update, context):
    n = context.user_data.get("country_name","?")
    s = context.user_data.get("spawn_text","?")
    g = context.user_data.get("gov","Не выбран")
    r = context.user_data.get("religion","Не выбрана")
    c = context.user_data.get("capital","Не указана")
    e = context.user_data.get("ethnicity","Не указан")
    f = "✅" if context.user_data.get("flag_photo") else "❌"
    cur = context.user_data.get("currency","Не указана")
    cr = context.user_data.get("currency_rate","—")
    ex = context.user_data.get("extra_info","") or "Нет"
    text = (
...
async def show_summary_callback(query, context):
    ...
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✏️ Исправить", callback_data="change_something")],[InlineKeyboardButton("📨 Отправить", callback_data="submit_to_admin")]]), parse_mode="HTML"); return CONFIRM
...
async def change_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("✏️ Что исправить?", reply_markup=InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📝 Дополнение", callback_data="change_extra")],
        [InlineKeyboardButton("✅ ОК", callback_data="submit_to_admin")]
    ]), parse_mode="HTML"); return CHANGE_MENU

async def change_name_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("🏛 Новое название:", parse_mode="HTML"); return CHANGE_NAME
...
async def change_spawn_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("🗺 Новое название/описание страны:", parse_mode="HTML"); return CHANGE_SPAWN
async def change_spawn_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo: context.user_data["spawn_photo"] = update.message.photo[-1].file_id; context.user_data["spawn_text"] = "(фото)"
    elif update.message.text: context.user_data["spawn_text"] = update.message.text; context.user_data["spawn_photo"] = None
    return await show_summary(update, context)
async def change_gov_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("🏛 Выбери новый госстрой:", reply_markup=gov_keyboard()); return CHANGE_GOV
...
async def change_religion_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("🕌 Выбери новую религию:", reply_markup=religion_keyboard()); return CHANGE_RELIGION
async def change_religion_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    rel = q.data.replace("relpick_","")
    if rel == "atheism":
        context.user_data["religion"] = "Атеизм"
        await q.edit_message_text("🚫 Выбери режим:", reply_markup=atheism_keyboard(), parse_mode="HTML")
...
async def change_atheism_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["religion"] = f"Атеизм ({ATHEISM_MODES[idx]})"
    return await show_summary_callback(q, context)
async def change_capital_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("🏙 Новая столица:", parse_mode="HTML"); return CHANGE_CAPITAL
...
async def change_ethnicity_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("👥 Выбери народ:", reply_markup=ethnicity_keyboard()); return CHANGE_ETHNICITY
async def change_ethnicity_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    context.user_data["ethnicity"] = q.data.replace("ethpick_","")
    return await show_summary_callback(q, context)
async def change_flag_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("🏴 Новый флаг:", parse_mode="HTML"); return CHANGE_FLAG
async def change_flag_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return await show_summary(update, context)
async def change_currency_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("💱 Напиши название валюты или нажми «Пропустить»:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_currency_change")]]), parse_mode="HTML"); return CHANGE_CURRENCY
async def change_currency_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["currency"] = update.message.text
    await update.message.reply_html("💱 Напиши курс к доллару (минимум 10):")
    return CURRENCY_RATE
async def skip_currency_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    context.user_data["currency"] = "Автоматически"
    context.user_data["currency_rate"] = "—"
    return await show_summary_callback(q, context)
async def change_extra_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("📝 Дополнение или «Пропустить»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra_change")]]), parse_mode="HTML"); return CHANGE_EXTRA
async def change_extra_done(update: Update, context: ContextTypes.DEFAULT_TYPE): context.user_data["extra_info"] = update.message.text or ""; return await show_summary(update, context)
async def skip_extra_change(update: Update, context: ContextTypes.DEFAULT_TYPE): context.user_data["extra_info"] = ""; return await show_summary_callback(update.callback_query, context)
...
async def submit_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("📨 <b>Анкета отправлена!</b>\n\nОжидай решения ИМПЕРАТОРА.", parse_mode="HTML")
    ...
    if ADMIN_ID:
        try:
            admin_text = (
                ...
                f"🗺 Страна: {data['spawn_text']}\n"
                f"🏛 Госстрой: {data['gov']}\n"
                f"🕌 Религия: {data['religion']}\n"
                f"🏙 Столица: {data['capital']}\n"
                f"👥 Народ: {data['ethnicity']}\n"
                f"💱 Валюта: {data['currency']} (курс: {data['currency_rate']})\n"
                f"📝 Дополнение: {data['extra_info'] or 'Нет'}"
            )
            admin_kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Одобрить", callback_data=f"approve_{u.id}"),InlineKeyboardButton("❌ Отказать", callback_data=f"reject_menu_{u.id}")]])
            await context.bot.send_message(chat_id=ADMIN_ID, text=admin_text, reply_markup=admin_kb, parse_mode="HTML")
            if data["spawn_photo"]: await context.bot.send_photo(chat_id=ADMIN_ID, photo=data["spawn_photo"], caption="🗺 Карта страны")
            if data["flag_photo"]: await context.bot.send_photo(chat_id=ADMIN_ID, photo=data["flag_photo"], caption="🏴 Флаг")
        except: pass

    for ank_id in anketers:
        try:
            await context.bot.send_message(chat_id=ank_id, text=f"📢 <b>НОВАЯ ЗАЯВКА</b>\n\n👤 @{u.username or 'нет'}\n🏛 Название: {data['name']}\n🗺 Страна: {data['spawn_text']}\n🏛 Госстрой: {data['gov']}\n🕌 Религия: {data['religion']}\n🏙 Столица: {data['capital']}\n👥 Народ: {data['ethnicity']}\n💱 Валюта: {data['currency']} (курс: {data['currency_rate']})\n📝 Дополнение: {data['extra_info'] or 'Нет'}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Одобрить", callback_data=f"approve_{u.id}"),InlineKeyboardButton("❌ Отказать", callback_data=f"reject_menu_{u.id}")]]), parse_mode="HTML")
            if data["spawn_photo"]: await context.bot.send_photo(chat_id=ank_id, photo=data["spawn_photo"], caption="🗺 Карта страны")
            if data["flag_photo"]: await context.bot.send_photo(chat_id=ank_id, photo=data["flag_photo"], caption="🏴 Флаг")
        except: pass
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    country_counter[6] = country_counter.get(6,0) + 1; cn = country_counter[6]; now = datetime.now().strftime("%d.%m.%Y %H:%M")
    countries[uid] = {
    ...
    await q.edit_message_text(f"✅ <b>{data['name']}</b> одобрена! (№{cn})\n🏛 {data.get('gov','?')}\n🕌 {data.get('religion','?')}\n🪨 {', '.join(res)}", parse_mode="HTML")
    try:
        await context.bot.send_message(chat_id=int(uid), text=f"🎉 <b>{data['name']}</b> создана!\n🔢 №{cn}\n🏛 {data.get('gov','?')}\n🕌 {data.get('religion','?')}\n🏙 {data.get('capital','?')}\n👥 {data.get('ethnicity','?')}\n💱 {data.get('currency','?')}\n🪨\n" + "\n".join(f"• {r}" for r in res), parse_mode="HTML")
    except: pass
    if maper_ids:
        maper_orders[uid] = data
        maper_orders[uid]["date"] = now
        save_json(MAPER_ORDERS_FILE, maper_orders)
        for mid in maper_ids:
            try:
                await context.bot.send_message(chat_id=mid, text=f"🗺 <b>Новая работа!</b>\n🏛 {data['name']}\n👤 @{data['username']}\n🏛 {data.get('gov','?')}\n🕌 {data.get('religion','?')}\n🏙 {data.get('capital','?')}\n💱 {data.get('currency','?')}\n📅 {now}\n📝 {data.get('extra_info','Нет')}\nНажми /map", parse_mode="HTML")
                if data.get("spawn_photo"): await context.bot.send_photo(chat_id=mid, photo=data["spawn_photo"], caption="🗺 Карта страны")
    ...
    if REVIEW_CHAT_ID: await context.bot.send_message(chat_id=REVIEW_CHAT_ID, text=f"🎉 <b>{data['name']}</b> — новая страна!\n👤 @{data['username']}\n🔢 №{cn}\n🏛 {data.get('gov','?')}", parse_mode="HTML")
...
async def admin_reject_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("❌ Причина:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📏 Спавн не подходит", callback_data=f"reject_{uid}_1")],[InlineKeyboardButton("📛 Название не подходит", callback_data=f"reject_{uid}_2")],[InlineKeyboardButton("💩 Это не страна, а бред", callback_data=f"reject_{uid}_3")],[InlineKeyboardButton("🔙 Отмена", callback_data=f"cancel_reject_{uid}")]]), parse_mode="HTML")
...
async def admin_reject_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reasons = {"1":"📏 Спавн не подходит.","2":"📛 Название не подходит.","3":"💩 Это не страна, а бред."}
    ...
    try: await context.bot.send_message(chat_id=int(uid), text=f"❌ <b>Отказано.</b>\n{rt}", parse_mode="HTML")
    except: pass
...
async def admin_cancel_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in pending_countries:
        ...
        await q.edit_message_text(f"📢 @{data['username']}\n🏛 {data['name']}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Одобрить", callback_data=f"approve_{uid}"),InlineKeyboardButton("❌ Отказать", callback_data=f"reject_menu_{uid}")]]), parse_mode="HTML")

async def broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.from_user.id != ADMIN_ID: return ConversationHandler.END
    msg = update.message.text; context.user_data["broadcast_msg"] = msg
    await update.message.reply_text(f"📢 Отправить {len(all_users)} пользователям?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Да", callback_data="admin_broadcast_yes")],[InlineKeyboardButton("❌ Нет", callback_data="admin_broadcast_no")]]), parse_mode="HTML")
