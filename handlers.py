...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "stata_seasons": await q.edit_message_text("📅 Сезон 1 (1200)\nСезон 2 (1991)\nСезон 3 (2008)\n🌍 Сезон 4 (1991) — текущий", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
    elif d == "stata_help": await q.edit_message_text("🆘 /start → меню\nКарта в «map»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
        ...
        archive["season_4_archived"] = True; save_json(ARCHIVE_FILE, archive)
...
async def start_create_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text("🏛 Шаг 1/4: Название:", parse_mode="HTML"); return NAME

async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["country_name"] = update.message.text
    await update.message.reply_html("🗺 Шаг 2/4: Спавн\n⚠️ Карта в «map»\n📏 Как два Донбасса"); return SPAWN
...
async def get_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    else: await update.message.reply_html("❌ Текст или фото!"); return SPAWN
    await update.message.reply_html("🏴 Шаг 3/4: Флаг (ОБЯЗАТЕЛЬНО):"); return FLAG
...
async def get_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("📝 Шаг 4/4: Напиши дополнение или нажми «Пропустить»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra")]])); return EXTRA
...
async def change_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("✏️ Что исправить?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏛 Название", callback_data="change_name")],[InlineKeyboardButton("🗺 Спавн", callback_data="change_spawn")],[InlineKeyboardButton("🏴 Флаг", callback_data="change_flag")],[InlineKeyboardButton("📝 Дополнение", callback_data="change_extra")],[InlineKeyboardButton("✅ ОК", callback_data="submit_to_admin")]]), parse_mode="HTML"); return CHANGE_MENU
...
async def change_spawn_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("🗺 Новый спавн:", parse_mode="HTML"); return CHANGE_SPAWN
...
async def submit_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending_countries[str(u.id)] = data
    save_json(PENDING_FILE, pending_countries)
    await q.edit_message_text("📨 Анкета отправлена! Ожидай решения ИМПЕРАТОРА.", parse_mode="HTML")
    if ADMIN_ID:
        try:
            ...
            if data["spawn_photo"]: await context.bot.send_photo(chat_id=ADMIN_ID, photo=data["spawn_photo"], caption="🗺 Спавн")
            elif data["spawn_text"]: await context.bot.send_message(chat_id=ADMIN_ID, text=f"🗺 Спавн: {data['spawn_text']}")
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    country_counter[4] = country_counter.get(4,0) + 1; cn = country_counter[4]; now = datetime.now().strftime("%d.%m.%Y %H:%M")
    countries[uid] = {"name": data["name"], "number": cn, "username": data["username"], "date": now, "extra_info": data.get("extra_info","")}; save_json(COUNTRIES_FILE, countries)
    ...
    if maper_ids:
        maper_orders[uid] = {"name": data["name"], "username": data["username"], "spawn_text": data.get("spawn_text",""), "spawn_photo": data.get("spawn_photo"), "flag_photo": data.get("flag_photo"), "extra_info": data.get("extra_info",""), "date": now}
    ...
    if uq != ADMIN_ID: await context.bot.send_message(chat_id=ADMIN_ID, text=f"✅ Админ @{q.from_user.username} одобрил «{data['name']}»")
...
async def admin_reject_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("❌ Причина:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📏 Большой спавн", callback_data=f"reject_{uid}_1")],[InlineKeyboardButton("📛 Название", callback_data=f"reject_{uid}_2")],[InlineKeyboardButton("💩 Бред", callback_data=f"reject_{uid}_3")],[InlineKeyboardButton("🔙 Отмена", callback_data=f"cancel_reject_{uid}")]]), parse_mode="HTML")
...
async def admin_reject_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reasons = {"1":"📏 Большой спавн.","2":"📛 Название не подходит.","3":"💩 Это не страна, а бред."}
    ...
    if uq != ADMIN_ID: await context.bot.send_message(chat_id=ADMIN_ID, text=f"❌ Админ @{q.from_user.username} отклонил ({rt})")
