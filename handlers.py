...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "enter_rp": await q.edit_message_text("🤖 Проверка", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я не бот", callback_data="verify_human")],[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "about_game": await q.edit_message_text("🍎 Лучшее РП!\n👑 ИМПЕРАТОР создал мир.\n📅 1936 год.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="back_to_start")]]), parse_mode="HTML")
    ...
    elif d == "stata_seasons": await q.edit_message_text("📅 Сезон 1 (1200)\nСезон 2 (1991)\nСезон 3 (2008)\nСезон 4 (1991)\nСезон 5 (1698)\nСезон 6 (2008)\n🌍 Сезон 7 (1936) — текущий", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
    elif d == "stata_help": await q.edit_message_text("🆘 /start → меню\nВыбери реальную страну 1936 года", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="stata_back")]]), parse_mode="HTML")
        ...
        if uid != ADMIN_ID: return
        text = "🌍\n" + "\n".join(f"🏛 {c['name']} | @{c['username']} | ID:{u} | 🏛{c.get('gov','?')}" for u, c in countries.items()) if countries else "Нет."
        await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="admin_menu")]]), parse_mode="HTML")
        ...
        archive["season_7_archived"] = True; save_json(ARCHIVE_FILE, archive)
...
async def anketer_add_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try: await context.bot.send_message(chat_id=new_id, text="📋 Вы назначены анкетчиком!")
        except: pass
...
async def edit_name_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sid in countries:
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"✏️ Страна переименована в «{update.message.text}»")
...
async def start_create_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text("🏛 <b>Шаг 1/8:</b> Напиши название своей страны:", parse_mode="HTML"); return NAME

async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["country_name"] = update.message.text
    await update.message.reply_html("🗺 <b>Шаг 2/8:</b> Напиши какой страной ты управляешь (реальная страна 1936 года).\n\n<i>Просто напиши название страны.</i>")
    return SPAWN
...
async def gov_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text("🏛 Шаг 3/8: Выбери госстрой:", reply_markup=gov_keyboard()); return GOV

async def gov_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text(f"✅ Госстрой: <b>{context.user_data.get('gov','?')}</b>", parse_mode="HTML")
    await q.message.reply_html(
        "🕌 <b>Шаг 4/8:</b> Выбери государственную религию.\n\n"
        "<i>Это основная религия страны, не значит что все 100% её исповедуют.</i>",
...
async def rel_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rel == "atheism":
        ...
        await q.edit_message_text(
            ...
            "<i>Внимание: в странах без религии социальная связь населения обычно ниже.</i>\n\n"
            "Выбери режим:",
    ...
    await q.edit_message_text(f"🕌 Ты выбрал: <b>{rel}</b>", reply_markup=kb, parse_mode="HTML"); return CONFIRM_RELIGION
...
async def atheism_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"✅ Выбрано", parse_mode="HTML")
    await q.message.reply_html("🏙 <b>Шаг 5/8:</b> Напиши название столицы:")
    return CAPITAL

async def rel_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await q.edit_message_text(f"✅ Религия: <b>{context.user_data.get('religion','?')}</b>", parse_mode="HTML")
    await q.message.reply_html("🏙 <b>Шаг 5/8:</b> Напиши название столицы:")
    return CAPITAL
...
async def get_capital(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("👥 <b>Шаг 6/8:</b> Напиши какой народ населяет твою страну.\n\n<i>Можно написать реальный или выдуманный народ.</i>")
    return ETHNICITY

async def eth_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["ethnicity"] = update.message.text
    await update.message.reply_html("🏴 <b>Шаг 7/8:</b> Отправь флаг страны <b>(ОБЯЗАТЕЛЬНО)</b>:", parse_mode="HTML")
    return FLAG
...
async def get_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    else: await update.message.reply_html("❌ Флаг обязателен!"); return FLAG
    await update.message.reply_html(
        "💱 <b>Шаг 8/8:</b> Напиши название валюты.\n\n"
        "<i>• Нельзя доллар или реальные валюты.\n"
        "• Если сезон про реальные страны — напиши «авто» для автовалюты.\n"
        "• Можно придумать свою.</i>"
    )
    return CURRENCY
...
async def get_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html("📝 Напиши дополнение или нажми «Пропустить»", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra")]]))
    return EXTRA
...
async def show_summary(update, context):
    text = (
        f"📋 <b>Проверь:</b>\n\n"
        f"🏛 Название: {context.user_data.get('country_name','?')}\n"
        f"🗺 Страна: {context.user_data.get('spawn_text','?')}\n"
        f"🏛 Госстрой: {context.user_data.get('gov','?')}\n"
        f"🕌 Религия: {context.user_data.get('religion','?')}\n"
        f"🏙 Столица: {context.user_data.get('capital','?')}\n"
        f"👥 Народ: {context.user_data.get('ethnicity','?')}\n"
        f"🏴 Флаг: {'✅' if context.user_data.get('flag_photo') else '❌'}\n"
        f"💱 Валюта: {context.user_data.get('currency','?')} ({context.user_data.get('currency_rate','—')})\n"
        f"📝 Дополнение: {context.user_data.get('extra_info','Нет')}\n\nВсё верно?"
    )
...
async def change_spawn_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("🗺 Какая страна?", parse_mode="HTML"); return CHANGE_SPAWN
async def change_spawn_done(update: Update, context: ContextTypes.DEFAULT_TYPE): context.user_data["spawn_text"] = update.message.text; return await show_summary(update, context)
...
async def change_currency_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return CURRENCY_RATE
async def change_extra_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.edit_message_text("📝 Дополнение:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭ Пропустить", callback_data="skip_extra_change")]]), parse_mode="HTML"); return CHANGE_EXTRA
...
async def skip_extra_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    context.user_data["extra_info"] = ""
    return await show_summary_callback(q, context)
...
async def submit_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("📨 <b>Анкета отправлена!</b>", parse_mode="HTML")
    ...
    if ADMIN_ID:
        try:
            admin_text = (
                ...
                f"💱 Валюта: {data['currency']} ({data['currency_rate']})\n"
                f"📝 {data['extra_info'] or 'Нет'}"
            )
            ...
            if data.get("spawn_photo"): await context.bot.send_photo(chat_id=ADMIN_ID, photo=data["spawn_photo"], caption="🗺")
    ...
    for ank_id in anketers:
        try:
            await context.bot.send_message(chat_id=ank_id, text=f"📢 <b>НОВАЯ ЗАЯВКА</b>\n👤 @{u.username}\n🏛 {data['name']}\n🗺 {data['spawn_text']}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅", callback_data=f"approve_{u.id}"),InlineKeyboardButton("❌", callback_data=f"reject_menu_{u.id}")]]), parse_mode="HTML")
            if data.get("spawn_photo"): await context.bot.send_photo(chat_id=ank_id, photo=data["spawn_photo"])
            if data.get("flag_photo"): await context.bot.send_photo(chat_id=ank_id, photo=data["flag_photo"])
        except: pass
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    country_counter[7] = country_counter.get(7,0) + 1; cn = country_counter[7]; now = datetime.now().strftime("%d.%m.%Y %H:%M")
    countries[uid] = {
    ...
    await q.edit_message_text(f"✅ {data['name']} одобрена! (№{cn})\n🪨 {', '.join(res)}", parse_mode="HTML")
    ...
    if maper_ids:
        ...
        for mid in maper_ids:
            try:
                ...
                if data.get("spawn_photo"): await context.bot.send_photo(chat_id=mid, photo=data["spawn_photo"])
                if data.get("flag_photo"): await context.bot.send_photo(chat_id=mid, photo=data["flag_photo"])
            except: pass
...
async def admin_reject_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("❌ Причина:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📏 Спавн", callback_data=f"reject_{uid}_1")],[InlineKeyboardButton("📛 Название", callback_data=f"reject_{uid}_2")],[InlineKeyboardButton("💩 Бред", callback_data=f"reject_{uid}_3")],[InlineKeyboardButton("🔙 Отмена", callback_data=f"cancel_reject_{uid}")]]), parse_mode="HTML")
