...
async def start_create_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text("🏛 Шаг 1/8: Напиши название страны:", parse_mode="HTML"); return NAME

async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["country_name"] = update.message.text
    await update.message.reply_html("🗺 Шаг 2/8: Напиши какой страной управляешь (1936 год).")
    return SPAWN

async def get_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["spawn_text"] = update.message.text
    await update.message.reply_html("🏛 Шаг 3/8: Выбери госстрой:", reply_markup=gov_keyboard())
    return GOV
...
async def gov_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"🏛 Ты выбрал: <b>{context.user_data['gov']}</b>", reply_markup=kb, parse_mode="HTML"); return CONFIRM_GOV
...
async def submit_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ADMIN_ID:
        try:
            ...
            if data.get("flag_photo"): await context.bot.send_photo(chat_id=ADMIN_ID, photo=data["flag_photo"])
        except: pass
...
async def admin_reject_confirm(update: Update,
