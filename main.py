...
BACKUP_DIR = "backups"
COUNTRIES_FILE = "countries_s6.json"
PENDING_FILE = "pending_s6.json"
RESOURCES_FILE = "resources_s6.json"
ARCHIVE_FILE = "archive.json"
REVIEWS_FILE = "reviews_s6.json"
USERS_FILE = "users_s6.json"
            ...
            for line in f:
                ...
                if "|" in line and not line.startswith("|"):
                    ...
                    if len(parts) >= 2:
                        ...
                        if response:
                            answers[keywords] = answers.get(keywords, []) + [response]
    return answers
...
if maper_data:
    if "ids" in maper_data:
        ids = maper_data["ids"]
        maper_ids = set(ids) if isinstance(ids, list) else {ids}
        maper_id = list(maper_ids)[0] if maper_ids else None
    elif "id" in maper_data:
        mid = maper_data["id"]
        maper_ids = set(mid) if isinstance(mid, list) else {mid}
        maper_id = list(maper_ids)[0] if maper_ids else None
...
country_counter = {6: 0}
if archive and archive.get("season_6_archived", False):
    season_archived = True
if countries:
    country_counter[6] = max(c.get("number", 0) for c in countries.values())
else:
    country_counter[6] = 0
        ...
        try:
            ...
            conv_handler = ConversationHandler(
                ...
                states={
                    ...
                    h.CURRENCY: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.get_currency), CallbackQueryHandler(h.skip_currency, pattern="^skip_currency$")],
                    ...
                    h.CHANGE_MENU: [CallbackQueryHandler(h.change_name_start, pattern="^change_name$"), CallbackQueryHandler(h.change_spawn_start, pattern="^change_spawn$"), CallbackQueryHandler(h.change_gov_start, pattern="^change_gov$"), CallbackQueryHandler(h.change_religion_start, pattern="^change_religion$"), CallbackQueryHandler(h.change_capital_start, pattern="^change_capital$"), CallbackQueryHandler(h.change_flag_start, pattern="^change_flag$"), CallbackQueryHandler(h.change_currency_start, pattern="^change_currency$"), CallbackQueryHandler(h.change_extra_start, pattern="^change_extra$"), CallbackQueryHandler(h.submit_to_admin, pattern="^submit_to_admin$")],
                    ...
                    h.CHANGE_CURRENCY: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.change_currency_done), CallbackQueryHandler(h.skip_currency_change, pattern="^skip_currency_change$")],
