...
BACKUP_DIR = "backups"
COUNTRIES_FILE = "countries_s7.json"
PENDING_FILE = "pending_s7.json"
RESOURCES_FILE = "resources_s7.json"
ARCHIVE_FILE = "archive.json"
REVIEWS_FILE = "reviews_s7.json"
USERS_FILE = "users_s7.json"
...
def load_government():
    if os.path.exists("government.txt"):
        with open("government.txt", "r", encoding="utf-8") as f:
            return f.read()
...
country_counter = {7: 0}
if archive and archive.get("season_7_archived", False):
    season_archived = True
if countries:
    country_counter[7] = max(c.get("number", 0) for c in countries.values())
else:
    country_counter[7] = 0
...
h.load_government = load_government
        ...
        try:
            ...
            conv_handler = ConversationHandler(
                ...
                states={
                    ...
                    h.SPAWN: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.get_spawn)],
                    h.GOV: [CallbackQueryHandler(h.gov_pick, pattern="^govpick_"), CallbackQueryHandler(h.gov_info_handler, pattern="^gov_info$")],
                    h.CONFIRM_GOV: [CallbackQueryHandler(h.gov_confirm, pattern="^gov_confirm$"), CallbackQueryHandler(h.gov_back, pattern="^gov_back$"), CallbackQueryHandler(h.gov_info_handler, pattern="^gov_info$")],
                    h.RELIGION: [CallbackQueryHandler(h.rel_pick, pattern="^relpick_")],
                    h.CONFIRM_RELIGION: [CallbackQueryHandler(h.rel_confirm, pattern="^rel_confirm$"), CallbackQueryHandler(h.rel_back, pattern="^rel_back$")],
                    h.ATHEISM_MODE: [CallbackQueryHandler(h.atheism_pick, pattern="^atheism_")],
                    h.CAPITAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.get_capital)],
                    h.ETHNICITY: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.get_ethnicity)],
                    h.FLAG: [MessageHandler(filters.PHOTO | filters.Document.ALL, h.get_flag)],
                    h.CURRENCY: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.get_currency)],
                    h.CURRENCY_RATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.get_currency_rate)],
                    ...
                    h.CHANGE_MENU: [
                        CallbackQueryHandler(h.change_name_start, pattern="^change_name$"),
                        CallbackQueryHandler(h.change_spawn_start, pattern="^change_spawn$"),
                        CallbackQueryHandler(h.change_gov_start, pattern="^change_gov$"),
                        CallbackQueryHandler(h.change_religion_start, pattern="^change_religion$"),
                        CallbackQueryHandler(h.change_capital_start, pattern="^change_capital$"),
                        CallbackQueryHandler(h.change_ethnicity_start, pattern="^change_ethnicity$"),
                        CallbackQueryHandler(h.change_flag_start, pattern="^change_flag$"),
                        CallbackQueryHandler(h.change_currency_start, pattern="^change_currency$"),
                        CallbackQueryHandler(h.submit_to_admin, pattern="^submit_to_admin$")
                    ],
                    h.CHANGE_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.change_name_done)],
                    h.CHANGE_SPAWN: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.change_spawn_done)],
                    h.CHANGE_GOV: [CallbackQueryHandler(h.change_gov_done, pattern="^govpick_")],
                    h.CHANGE_RELIGION: [CallbackQueryHandler(h.change_religion_done, pattern="^relpick_"), CallbackQueryHandler(h.change_atheism_done, pattern="^atheism_")],
                    h.CHANGE_CAPITAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.change_capital_done)],
                    h.CHANGE_ETHNICITY: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.change_ethnicity_done)],
                    h.CHANGE_FLAG: [MessageHandler(filters.PHOTO | filters.Document.ALL, h.change_flag_done)],
                    h.CHANGE_CURRENCY: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.change_currency_done)],
                    ...
                    h.ANKETER_ADD: [MessageHandler(filters.TEXT & ~filters.COMMAND, h.anketer_add_id)],
                },
            ...
            app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.GROUPS, h.chat_message))
